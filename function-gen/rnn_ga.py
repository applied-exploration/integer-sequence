...
from models.rnn.ga_plain import GA_Plain
...
class RNN_GA_Unified(LearningAlgorithm):
    ...
    def __init__(self, symbols: List[str], output_sequence_length: int, encoded_seq_length: int, mutation_rate: int, num_epochs: int, population_size: int, input_size: int, output_size: int, hidden_size: int = 256, learning_rate: float = 0.01, calc_magnitude_on=False):
        ...
        self.rnn = RNN_Plain(symbols=self.symbols, output_sequence_length=output_sequence_length, encoded_seq_length=encoded_seq_length, num_epochs=num_epochs,
    ...
    def infer(self, input_lang: Lang, output_lang: Lang, data: List[List[int]]) -> List[str]:
        self.encoded_best_guesses = self.rnn.infer(
            input_lang, output_lang, data)
        self.decoded_best_guesses = [eq_encoder(
            output_eq) for output_eq in encoded_best_guesses]
        self.init_population = self.create_varied_population(
            self.decoded_best_guesses)

        self.ga = GA_Plain(symbols=self.symbols, output_sequence_length=self.output_sequence_length,
                           encoded_seq_length=self.encoded_seq_length, mutation_rate=self.mutation_rate, num_epochs=self.num_epochs, population_size=self.population_size, init_population=self.init_population)
